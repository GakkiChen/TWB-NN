...
from config import *
from utils import *
from my_image_dataset_score import *
...
class AMNet:
    ...
    def init(self, hps):
        ...
        self.experiment_path = os.path.join(self.data_dir, hps.experiment_name) + '_score'
    ...
    def load_dataset(self, anno_dir, img_dir, train=True, batch_size=512, 
                    drop_last=True, num_workers=10):

        dataset = VPADataset(anno_dir, img_dir,
    ...
    def train_epoch(self, epoch, train_loader):
        ...
        if self.hps.front_end_cnn != 'ResNet50FT':
            for p in self.model.module.core_cnn.parameters():
        ...
        if params.use_cuda:
            ...
            params.mem_loc_w = params.mem_loc_w.to(device)
    ...
    def train(self):
        ...
        train_anno_dir = '/home/chen/work/visual_privacy/vpa/data/annos/train2017_anno/train2017'
        test_anno_dir = '/home/chen/work/visual_privacy/vpa/data/annos/test2017_anno/test2017'
        train_img_dir = '/home/chen/work/visual_privacy/vpa/data'
        test_img_dir = '/home/chen/work/visual_privacy/vpa/data'

        train_data_loader = self.load_dataset(train_anno_dir, train_img_dir, train=True, batch_size=self.hps.train_batch_size)

        test_data_loader = self.load_dataset(test_anno_dir, test_img_dir, train=False, batch_size=self.hps.test_batch_size)
        ...
        for epoch in range(self.hps.epoch_start + 1, self.hps.epoch_start + self.hps.epoch_max):
            ...
            self.save_checkpoint(os.path.join(self.get_experiment_path(), 'checkpoint_weights.pkl'))
            if rc > best_rc:
                best_rc = rc
                self.save_checkpoint(os.path.join(self.get_experiment_path(), 'best_weights.pkl'))
...
# Evaluation
    ...
    def predict_memorability(self, images_path):
        ...
        batch_size = 64
        if len(test_data_loader.dataset) < batch_size:
            batch_size = len(test_data_loader.dataset)
            print("Reducing batch size from ", self.hps.test_batch_size, "to",batch_size)

        num_workers = 8
        test_data_loader = self.load_dataset(test_anno_dir, test_img_dir, train=False, batch_size=batch_size,
                                            drop_last=False,)
        print('loading finished')
        pr = self.predict(test_data_loader)
